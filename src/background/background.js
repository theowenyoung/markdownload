...
function textReplace(string, article, disallowedChars = null, options) {
  for (const key in article) {
    if (article.hasOwnProperty(key) && key != "content") {
      let s = (article[key] || "") + "";
      if (s && disallowedChars)
        s = this.generateValidFileName(s, disallowedChars);
  ...
  if (matches && matches.forEach) {
    matches.forEach((match) => {
      ...
      let dateString = moment(now).format(format);
      if (timezone) {
        const momentInstance = moment(now);
        dateString = momentInstance.tz(timezone).format(format);
      }
...
async function convertArticleToMarkdown(article, downloadImages = null) {
  ...
  if (options.includeTemplate) {
    options.frontmatter =
      textReplace(options.frontmatter, article, null, options) + "\n";
    options.backmatter =
      "\n" + textReplace(options.backmatter, article, null, options);
  } else {
...
async function notify(message, sender) {
  ...
  if (message.type == "clip") {
    ...
    await browser.runtime.sendMessage(displayMessage);
  } else if (message.type === "changeOptions") {
  }
...
async function getArticleFromDom(domString, lang) {
  // parse the dom
  ...
  article.pageTitle1 = (titles[1] || titles[0]).trim();

  article.pageTitle2 = titles[0].trim();
  // try to get pageTitleWithoutSiteName
  if (lang) {
    article.lang = lang;
  } else {
    if (dom.documentElement.getAttribute("lang")) {
      article.lang = dom.documentElement.getAttribute("lang");
    }
  }

  article.pageTitleWithoutSiteName = trimPageTitle(dom.title);
  // and some URL info
...
async function getArticleFromContent(tabId, selection = false) {
  ...
  const language = await browser.tabs.detectLanguage(tabId);

  // get tab language

  // make sure we actually got a valid result
  if (results && results[0] && results[0].dom) {
    const article = await getArticleFromDom(results[0].dom, language);
